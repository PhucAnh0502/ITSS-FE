...
function NavigationBar({ activeTab, onTabChange }) {
  return (
    <nav className="w-full bg-white/80 backdrop-blur-md border-b border-blue-100 sticky top-0 z-50 px-6 shadow-sm shadow-blue-500/5" aria-label={LOCALIZATION.aria.mainNavigation}>
      <div className="flex items-center justify-between max-w-6xl mx-auto h-16">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button className="w-10 h-10 flex items-center justify-center rounded-full bg-transparent border-none cursor-pointer text-gray-600 hover:bg-blue-50 hover:text-blue-600 transition-colors" aria-label={LOCALIZATION.aria.notification}>
            <Bell size={20} />
          </button>
          <button className="w-10 h-10 flex items-center justify-center rounded-full bg-brand-gradient text-white border-none cursor-pointer shadow-sm shadow-blue-500/30 hover:opacity-90 transition-opacity" aria-label={LOCALIZATION.aria.profile}>
            <User size={20} />
          </button>
        </div>
      </div>
